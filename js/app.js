...
function updateCart() {
  ...
  const cleanedCart = catalogReady
    ? normalizedCart.filter(item =>
        products.some(product => productIdKey(product.id) === productIdKey(item.id))
      )
    : normalizedCart;

  const cartChanged = JSON.stringify(cleanedCart) !== JSON.stringify(cart);
  cart = cleanedCart;

  if (catalogReady && cartChanged) {
    saveData();
  } else {
    localStorage.setItem("im_cart", JSON.stringify(cart));
  }
