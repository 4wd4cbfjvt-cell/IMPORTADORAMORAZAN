...
function exportProducts() {
  ...
  link.href = url;
  link.download = `importadora-morazan-catalogo-${date}.json`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);

  alert("Catálogo guardado. Este archivo nos sirve como respaldo y para publicar los productos después.");
}
...
async function login() {
  ...
  try {
    ...
    alert("Este navegador no puede verificar la contraseña de forma segura. Usa Chrome, Safari, Edge o Firefox actualizado.");
    return;
  }
